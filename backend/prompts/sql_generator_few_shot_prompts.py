...
class FewShotPrompt:
    ...
    def __init__(self):
        self.examples = [
            ...
            {
                'question': 'Remove all customers who have paid their loan.',
                'sql': "Sorry, I can only generate read-only (SELECT) queries. Data modification operations like DELETE or REMOVE are not allowed."
            },
            {
                'question': 'Delete all accounts with zero balance.',
                'sql': "Sorry, I can only generate read-only (SELECT) queries. Data modification operations like DELETE are not allowed."
            },
            {
                'question': 'Drop the loans table.',
                'sql': "Sorry, I can only generate read-only (SELECT) queries. Data modification operations like DROP are not allowed."
            },
            {
                'question': 'Update the status of all overdue loans to closed.',
                'sql': "Sorry, I can only generate read-only (SELECT) queries. Data modification operations like UPDATE are not allowed."
            },
            {
                'question': 'Insert a new customer into the database.',
                'sql': "Sorry, I can only generate read-only (SELECT) queries. Data modification operations like INSERT are not allowed."
            },
            {
                'question': 'Alter the accounts table to add a new column.',
                'sql': "Sorry, I can only generate read-only (SELECT) queries. Data modification operations like ALTER are not allowed."
            },
            {
                'question': 'Truncate the transactions table.',
                'sql': "Sorry, I can only generate read-only (SELECT) queries. Data modification operations like TRUNCATE are not allowed."
            },
            {
                'question': 'Erase all loan payment records.',
                'sql': "Sorry, I can only generate read-only (SELECT) queries. Data modification operations like ERASE are not allowed."
            },
            {
                'question': 'Revoke access from a user.',
                'sql': "Sorry, I can only generate read-only (SELECT) queries. Data modification operations like REVOKE are not allowed."
            },
            {
                'question': 'Grant admin privileges to a user.',
                'sql': "Sorry, I can only generate read-only (SELECT) queries. Data modification operations like GRANT are not allowed."
            },
            # ✅ CRITICAL: Time-based relative comparison example
            {
                ...
                'sql': (
                    ...
                    "AND ABS(DATE_PART('day', a.open_date::timestamp - l.start_date::timestamp)) <= 90 "
                    "AND ABS(DATE_PART('day', cc.issue_date::timestamp - l.start_date::timestamp)) <= 90;"
                )
            },
            {
            ...
            {
                'question': 'List anybody who has a plastic card and has account.',
                'sql': (
                    "SELECT DISTINCT c.customer_id, c.first_name, c.last_name "
                    "FROM customers c "
                    "JOIN credit_cards cc ON c.customer_id = cc.customer_id "
            ...
            # --- CUSTOM: Failed followed by Successful Loan Payment ---
            {
            ...
            # --- CUSTOM: Late payment followed by on-time payment ---
            {
                'question': 'List customers who had a late loan payment followed by an on-time payment.',
                'sql': (
                    "SELECT DISTINCT c.customer_id, c.first_name, c.last_name "
                    "FROM customers c "
                    "JOIN loans l ON c.customer_id = l.customer_id "
                    "JOIN loan_payments lp1 ON l.loan_id = lp1.loan_id "
                    "JOIN loan_payments lp2 ON l.loan_id = lp2.loan_id "
                    "WHERE lp1.status = 'late' AND lp2.status = 'on-time' "
                    "AND lp1.payment_date < lp2.payment_date;"
                )
            },
            # --- CUSTOM: Missed payment followed by late payment ---
            {
                'question': 'List customers who had a missed loan payment followed by a late payment.',
                'sql': (
                    "SELECT DISTINCT c.customer_id, c.first_name, c.last_name "
                    "FROM customers c "
                    "JOIN loans l ON c.customer_id = l.customer_id "
                    "JOIN loan_payments lp1 ON l.loan_id = lp1.loan_id "
                    "JOIN loan_payments lp2 ON l.loan_id = lp2.loan_id "
                    "WHERE lp1.status = 'missed' AND lp2.status = 'late' "
                    "AND lp1.payment_date < lp2.payment_date;"
                )
            },
            {
                'question': 'Show the trend of new credit cards issued per month over the last two years.',
                'sql': (
                    "WITH monthly_cards AS ("
                    ...
                    "    GROUP BY DATE_TRUNC('month', issue_date) "
                    ") "
                    "SELECT month, COALESCE(num_cards, 0) AS num_cards "
                    "FROM generate_series(" 
                    "    DATE_TRUNC('month', CURRENT_DATE - INTERVAL '2 years'), "
                    "    DATE_TRUNC('month', CURRENT_DATE - INTERVAL '1 month'), "
                    "    INTERVAL '1 month' "
                    ") month "
                    "LEFT JOIN monthly_cards USING (month) "
                    "ORDER BY month;"
                )
            },
        ]
